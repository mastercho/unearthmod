...
| `greynoise_asset` | Passive | Yes — `GREYNOISE_API_KEY` | 0.65 | GreyNoise GNQL search — not a cert pivot; queries `/v2/experimental/gnql` for every IP GreyNoise's planetary sensor mesh has observed whose `metadata.rdns` contains the target apex or whose `metadata.organization` matches the target name, and emits the non-CDN IPs as origin candidates; the corpus is *sensor-observed internet noise* (every IP GreyNoise's mesh has interacted with in the last ~90 days, tagged with rDNS/org/ASN metadata), a different lineage than every other backend — a forgotten origin (`origin.`, `direct.`, `mail.`, `staging.`) whose rDNS still advertises the apex and that has talked to anyone recently surfaces here even when no cert engine, asset crawler, or passive-DNS feed recorded it; GNQL is a paid Investigate/Enterprise feature, the Community tier is gracefully skipped |
...
| `favicon_hash` | Active | Yes — `SHODAN_API_KEY` or `CENSYS_PLATFORM_PAT` | 0.75 | Favicon MurmurHash3 pivot — fetches declared page icon links plus `/favicon.ico`, queries Shodan/Censys for hosts sharing any matching favicon |
...
>
> **GreyNoise note:** `greynoise_asset` needs `GREYNOISE_API_KEY` (generated from your GreyNoise account's [API key page](https://viz.greynoise.io/account/api-key); the `UNEARTH_GREYNOISE_API_KEY` alias is also accepted); without it the technique is skipped. The GNQL search endpoint sits behind the paid Investigate / Enterprise plans — when the Community-tier key cannot reach the endpoint, or the request allowance is exhausted, the API answers `HTTP 401`/`402`/`403`/`429` (or a `message`/`error` envelope mentioning `feature is not enabled`, `upgrade`, `quota`, `rate limit`, `subscription`, `payment required`, `trial`), which the technique treats as a clean tier-insufficient skip rather than a failure; a missing/invalid key (`HTTP 401`, or a `message`/`error` envelope mentioning `Invalid API key`/`unauthorized`) degrades to a clean missing-key skip. **Unlike every other backend in the suite — the eight certificate-fingerprint engines, the four asset crawlers (FullHunt, ZoomEye, Chaos, FullHunt-shape backends), and the three passive-DNS / scan-record feeds (VirusTotal, URLScan, OTX) — GreyNoise is a *sensor-observed-noise* pivot.** Its corpus is every IP GreyNoise's planetary sensor mesh has interacted with in the last ~90 days, tagged with reverse-DNS, organization, and ASN metadata. The technique issues one GNQL query — `metadata.rdns:*<target> OR metadata.organization:"<target>"` — and emits the non-CDN, deduplicated IPs as origin candidates, preferring rDNS-match evidence over org-match evidence (rDNS is the stronger signal that the IP is actually under the target). Pagination is handled via GreyNoise's `scroll` cursor with a hard 10-page ceiling to bound the fetch. Its value is corpus diversity from a yet-different axis: a forgotten origin (`origin.`, `direct.`, `mail.`, `staging.`) whose rDNS still advertises the apex *and* that has generated any internet traffic recently surfaces here even when no cert engine, asset crawler, or passive-DNS feed recorded it — the IP must be *talking* to be in GreyNoise, and the rDNS/org metadata GreyNoise attaches turns that into a clean origin-discovery pivot.
>
