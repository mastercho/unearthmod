...
**Why it complements the other engines:** LeakIX runs its own continuous internet-wide scan and exposure index, overlapping only partially with Shodan, Censys, FOFA, Netlas, and Criminal IP. Like Shodan, LeakIX indexes the leaf cert's **SHA-1** fingerprint rather than the SHA-256 the Censys/FOFA/Netlas/Criminal IP pivots rely on, so it broadens reach and corroborates the SHA-1 pivot from another source. A misconfigured origin that leaks its real certificate may surface in LeakIX when it is absent from the others — the value is coverage diversity, not redundancy.
...
**Tier:** Passive | **Weight:** 0.65 | **API key:** `GREYNOISE_API_KEY`

**What it does:** Queries GreyNoise's GNQL (GreyNoise Query Language) search endpoint for every IP GreyNoise's planetary sensor mesh has observed whose reverse-DNS metadata contains the target apex domain *or* whose organization-metadata label matches the target name, and surfaces the non-CDN IPs as origin candidates. **This is not a certificate-fingerprint pivot, an asset-crawler pivot, or a passive-DNS pivot.** It is a *sensor-observed-noise* pivot: GreyNoise indexes every IP that has generated internet traffic its sensors observed in the recent past (~90-day rolling window in the default view), tagged with reverse-DNS, organization, ASN, and classification metadata. The technique pivots on that metadata to find IPs that advertise themselves as part of the target's footprint *and* are actually live on the public internet.

**Data source:** GreyNoise GNQL search API (`https://api.greynoise.io/v2/experimental/gnql`). Authenticated via the `key` header. The query is rendered as `metadata.rdns:*<target> OR metadata.organization:"<target>"` — a single GNQL expression that combines the rDNS-substring and organization-exact pivots so both signals are exercised in one request. The response envelope is `{"complete":bool,"count":N,"data":[{"ip","metadata":{"rdns","organization","asn"}},…],"scroll":"…","message":"…"}`. Pagination uses the `scroll` cursor; the technique loops until `complete:true` or the scroll cursor is empty, with a hard 10-page ceiling to bound the fetch.

**How it differs from every other backend:** Eight cert engines pivot on the target's *current* leaf certificate — they miss origins that never reused it. Four crawl/asset enumerators (FullHunt, ZoomEye, Chaos, plus FullHunt-shape backends) pivot on what active crawlers and community datasets catalogued under the apex. Three telemetry pivots (VirusTotal, URLScan, OTX) pivot on URL/file/scan submissions and threat-intel feeds. GreyNoise pivots on a fundamentally different axis: **the IP must be live and noisy enough to have hit a GreyNoise sensor recently, and its rDNS or org metadata must advertise the target.** That intersection — *currently-live + self-advertising* — is exactly the shape of a forgotten origin (`origin.example.com`, `mail.example.com`, `staging.example.com`) whose PTR record still points under the apex and that is still serving traffic even though it was supposed to have been retired behind the CDN.

**Evidence ranking in candidates:** Each candidate's evidence string prefers the most-specific reason for surfacing. If the IP's `metadata.rdns` contains the target apex (case-insensitive substring match), the evidence reads `rDNS <hostname>` — the strongest signal that the IP is genuinely under the target. Otherwise, if the `metadata.organization` field is populated, the evidence reads `org "<org-name>"` — a weaker but still actionable signal (the IP belongs to an organization GreyNoise has labelled as the target's). The fallback evidence is `GNQL match` for records where GreyNoise has neither rDNS nor org metadata but the GNQL query still returned a hit.

**Key requirement:** `GREYNOISE_API_KEY` must be present; without it the technique skips gracefully. GreyNoise's GNQL search is gated to the paid Investigate / Enterprise plans — the free Community tier answers `HTTP 401` / `402` / `403` for this endpoint, all of which the technique treats as a clean tier-insufficient skip (only the bare `401` is classified as a missing-key skip, since that is the canonical "your credentials are wrong" response). The `UNEARTH_GREYNOISE_API_KEY` alias is also accepted for backward compatibility with the codebase's `UNEARTH_`-prefixed convention. Quota / rate-limit responses (`HTTP 429`, or a 2xx carrying a `message`/`error` envelope mentioning `quota`, `rate limit`, `upgrade`, `subscription`, `plan`, `permission`, `payment required`, `trial`, or `feature is not enabled`) likewise degrade to a clean tier-insufficient skip. Bare `"ok"`/`"success"`/`"no results"` messages with an empty `data` array are treated as a benign empty success — not all empty-data responses indicate an error envelope.

**Limitations:** GNQL search is paid; deployments running on the Community tier never reach this technique's payload, only its tier-skip path. GreyNoise's metadata enrichment is best-effort — for fresh IPs that have not yet had reverse-DNS or organization metadata attached, the technique will not surface them even when the IP is technically under the apex. The rDNS-substring filter (`metadata.rdns:*<target>`) is intentionally permissive and will match any rDNS containing the target string anywhere — that is the correct behavior for catching `*.example.com`, `example-com.staging`, and similar variants, but it can occasionally surface IPs whose rDNS happens to share a substring with the target without actually being under it (e.g. a target of `example.com` could match an rDNS of `example.community.org`); the evidence string surfaces the full rDNS so operators can triage. The 10-page scroll ceiling caps the fetch at GreyNoise's default `data` array size × 10 — well past any realistic origin-discovery need but worth knowing for very high-volume apex domains.
...
**What it does:** Fetches the target baseline with a Chrome-like uTLS/HTTP2 client, then probes each candidate IP (from phase-1 techniques) over common web ports using both direct-IP access and `Host: target` with target SNI. Candidate probes do not follow redirects: a redirect back to the protected hostname would otherwise compare the baseline to itself and create a false confirmation. A response served directly by the candidate that mirrors the target's content, has compatible TLS/header signals, and lacks CDN-identifying headers is strong evidence that the IP is the real origin.
...
**Limitations:** Requires at least one phase-1 candidate IP. Origins running on non-standard ports outside the built-in probe list may not respond. Very aggressive rate limiting on the origin may cause false negatives.
...
**What it does:** Requests the common phpinfo endpoint paths from ProjectDiscovery's Nuclei `phpinfo-files` template and confirms pages with the same `PHP Extension` plus `PHP Version` matcher. When a phpinfo page is exposed, it extracts public non-CDN IPs from server-side fields such as `SERVER_ADDR` and `LOCAL_ADDR`. Cloudflare challenge responses are retried with the Chrome-like uTLS client and reported explicitly in verbose output if the retry remains blocked.

**Limitations:** Only fires when the phpinfo HTML is reachable from the machine running Unearth. A page that works in an interactive browser can still be unavailable to a VPS scanner when Cloudflare applies a managed challenge based on client IP or browser state. It deliberately ignores client-side fields such as `REMOTE_ADDR` so it does not report the scanner or proxy address.
